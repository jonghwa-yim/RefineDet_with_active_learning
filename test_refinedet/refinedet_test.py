import test_refinedet._init_paths
from test_refinedet.lib.fast_rcnn.test import single_scale_test_net, multi_scale_test_net_320, multi_scale_test_net_512
from test_refinedet.lib.fast_rcnn.config import cfg, cfg_from_file, cfg_from_list
from test_refinedet.lib.datasets.factory import get_imdb
import caffe
import os

from allinone_create_new_trainval import COCO_NAMEs


def run_refinedet_test(idx='', prj_path='', base_mode=0):
    GPU_ID = 1
    ...
    coco_path = prj_path + 'models/ResNet/coco/' + COCO_NAMEs[base_mode]['job_name'].format('320x320') + '/'
    ...
    if 'voc' in test_set:
        ...
        for key in keys:
            ...
            templine.append("%d\t    %.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\n"
                            %(key,value[0],value[1],value[2],value[3],value[4],value[5],value[6],value[7],value[8],value[9],value[10],value[11]))
        with open(path+'mAP' + str(idx) + '.txt', 'w+') as f:
            f.writelines(templine)
    print("#########################################################################################################")
    print("#########################################################################################################")
